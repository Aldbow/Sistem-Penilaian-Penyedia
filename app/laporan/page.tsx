...
export default function LaporanPage() {
  ...
  // Update data stability when relevant states change (not including paginatedData to avoid circular dependency)
  useEffect(() => {
  ...
  }, [currentPage, itemsPerPage, searchQuery, filterStatus, starFilter, sortOption, starSort, mostRatedFilter, dateSort])
  ...
  const filteredData = useMemo(() => {
    return penyediaData.filter(penyedia => {
      const matchesSearch = penyedia.namaPerusahaan.toLowerCase().includes(debouncedSearch.toLowerCase()) ||
        penyedia.npwp.includes(debouncedSearch)
  ...
  }, [penyediaData, debouncedSearch, filterStatus, starFilter, sortOption, starSort, mostRatedFilter, dateSort])
  ...
  // Memoize top performers to prevent unnecessary recalculations
            ...
            {paginatedData.length > 0 ? (
              <div className="space-y-4">
                {paginatedData.map((item, index) => (
                  <motion.div
                    key={item.id}
                    initial={isDataStable ? { opacity: 0, y: 20 } : false}
                    animate={isDataStable ? { opacity: 1, y: 0 } : false}
                    ...
                    <Card className="hover:shadow-2xl hover:shadow-slate-500/20 transition-all duration-500 hover:scale-[1.01] hover:-translate-y-1 bg-gradient-to-r from-white to-gray-50 dark:from-slate-800 dark:to-slate-700 group rounded-2xl border">
                      <CardContent className="p-5">
                        <div 
                          ...
                          onClick={async () => {
                            // Show loading state when opening modal
                            ...
                            try {
                              ...
                              if (response.ok) {
                                ...
                                // Still show the modal with available data if API fails
                                const penyediaWithDetails: PenyediaWithDetails = {
                                  ...item,
                                  penilaian: []
                                }
          ...
          <Card className="rounded-2xl shadow-lg">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {topPerformers.map((penyedia, index) => (
                  <motion.div
                    key={penyedia.id}
                    initial={isDataStable ? { opacity: 0, y: 20 } : false}
                    animate={isDataStable ? { opacity: 1, y: 0 } : false}
                  ...
                  >
                    <Card 
                      ...
                      onClick={() => {
                        ...
                        setSelectedPenyedia(penyediaWithDetails);
                        // Delay opening modal slightly to allow data preparation
                        setTimeout(() => {
                          setIsModalLoading(false);
                          setIsModalOpen(true);
                        }, 10);
